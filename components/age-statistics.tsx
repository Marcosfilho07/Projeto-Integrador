...
const chartConfig: ChartConfig = {
  media: { label: "Tempo médio", color: "var(--chart-1)" },
  participantes: { label: "Participantes", color: "var(--chart-2)" },
}
...
export function AgeStatistics({ activities, participants }: { activities: Activity[]; participants: Participant[] }) {
  ...
  useEffect(() => {
    if (!activities.some((activity) => activity.id === selectedId)) {
      setSelectedId(activities[0]?.id ?? "")
    }
  }, [activities, selectedId])
  const selectedActivity = activities.find((activity) => activity.id === selectedId) ?? activities[0]
  const items = useMemo(
    () => selectedActivity ? participants.filter((participant) => belongsToActivity(participant, selectedActivity)) : [],
  ...
  const ageDistribution = averages.map((item, index) => ({
    ...item,
  ...
  return <div className="flex flex-col gap-4">
    ...
    <div className="grid gap-4 lg:grid-cols-2"><Card><CardHeader><CardTitle>Tempo médio por faixa etária</CardTitle><CardDescription>Somente tempos válidos de {selectedActivity.name}.</CardDescription></CardHeader><CardContent><ChartContainer config={chartConfig} className="h-72 w-full"><BarChart accessibilityLayer data={averages.map((item) => ({ ...item, average: item.average ?? 0 }))}><CartesianGrid vertical={false} /><XAxis dataKey="group" /><YAxis tickFormatter={(value) => `${value}s`} /><Tooltip content={<ChartTooltipContent formatter={(value) => formatTime(Number(value))} />} /><Bar dataKey="average" fill="var(--color-media)" radius={6}>{averages.map((item) => <Cell key={item.group} fill={item.average === null ? "var(--muted)" : "var(--color-media)"} />)}</Bar></BarChart></ChartContainer></CardContent></Card><Card><CardHeader><CardTitle>Distribuição por faixa etária</CardTitle><CardDescription>Participantes vinculados a {selectedActivity.name}.</CardDescription></CardHeader><CardContent><div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_12rem] md:items-center"><ChartContainer config={chartConfig} className="mx-auto h-64 w-full max-w-72"><PieChart accessibilityLayer><Tooltip content={<ChartTooltipContent nameKey="group" />} /><Pie data={ageDistribution} dataKey="participantes" nameKey="group" innerRadius={58} outerRadius={88} paddingAngle={3} strokeWidth={2} label={({ group, participantes }) => participantes ? `${group}: ${participantes}` : ""}>{ageDistribution.map((item) => <Cell key={item.group} fill={item.fill} />)}</Pie></PieChart></ChartContainer><div className="flex flex-col gap-2">{ageDistribution.map((item) => <div key={item.group} className="flex items-center justify-between gap-3 text-sm"><span className="flex items-center gap-2"><span className="size-2.5 rounded-full" style={{ backgroundColor: item.fill }} />{item.group} anos</span><Badge variant="secondary">{item.count}</Badge></div>)}</div></div></CardContent></Card></div>
